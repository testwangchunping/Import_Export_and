...
suite=unittest.TestLoader().discover('H:\\selenium_test\\demo\\xbwq5\ExportTest\\testsuits',pattern='test_*.py')

if __name__=='__main__':

    #执行用例
    # 初始化一个HTMLTestRunner实例对象，用来生成报告（stream用来指定测试报告文件，title定义测试报告标题，description定义测试报告副标题）
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title=u"小步外勤导入导出项目测试报告", description=u"用例测试情况")
    # 开始执行测试套件中所组装的测试用例
    runner.run(suite)
    #关闭报告文件
    fp.close()
